...
2. **Cálculo de Puntos y Botones de Clasificación:** 
   - La web rota entre 3 botones principales: **Clasificación Grupos 📊** (Base + Aciertos de Grupos), **Clasificación Base 🔵** (Solo puntos de Partidos), y **Clasificación General 🥇** (Base + Grupos + Preguntas).
   ...
   - **Informes PDF y Descargas Originales:** La plataforma puede generar un PDF (`generateParticipantPDF`) por jugador que imprime 3 tablas detalladas: Partidos, Grupos (del 1º al 4º para asegurar los puntos de posición) y Preguntas. Además, se han añadido dos botones debajo del buscador que enlazan con el PDF original de cada jugador (mapeado a través de un diccionario interno en `api-engine.js`), permitiendo comparar su apuesta original de Grupos (y próximamente Eliminatorias) con sus resultados en la web.
...
3. **Panel de Administrador (Preguntas Especiales):**
   ...
   - **Tolerancia a errores ortográficos:** Ignora mayúsculas y tildes (`normalize("NFD")`).
   - **Multi-Respuestas por Empates:** Si una pregunta tiene empate (ej: equipo menos goleado), Lendo puede escribir las opciones en `official_answers.json` separadas por comas (ej: `"Mexico, Haiti"`). La web le sumará los puntos a cualquier jugador que haya puesto una de esas opciones.
...
6. **Fase 2 (Las Eliminatorias):** ACUERDO PARA LA TRANSICIÓN:
   - **Fase Final por Etapas (NUEVO ACUERDO):** La Fase Final NO se hará del tirón. Se hará eliminatoria por eliminatoria (primero Dieciseisavos, luego Octavos, etc.). Lendo pasará los pronósticos de Dieciseisavos primero. Una vez acaben, pasará los de Octavos, y así sucesivamente. Esto evita que los participantes pierdan equipos eliminados y no puedan puntuar en rondas avanzadas.
   ...
   - **Ingesta de Pronósticos (PDFs):** Se ha acordado que Lendo depositará los nuevos PDFs de cada ronda eliminatoria en una carpeta (ej. `participantes_fase2_dieciseisavos`). La Princesa usará sus scripts para leerlos masivamente y extraer los cruces sin que Lendo tenga que picarlos a mano en Excel.
   ...
   - **Interfaz Perfil de Jugador (Buscar Pronóstico):** Al desbloquear la Fase Final, se añadirá un nuevo botón ("Fase Final" o "Eliminatorias") en el perfil de usuario, junto a "Partidos", "Grupos" y "Preguntas". Mostrará el árbol de cruces o los partidos de la ronda actual, contrastando la predicción con la realidad y desglosando los puntos ganados, incluso si al principio arranca vacío a la espera de los datos.
...
- **Diagnóstico de Discrepancias con Excel Manual:** Si Lendo reporta diferencias entre los puntos de la web y los de su rival del Excel, las causas comprobadas son: (1) **El Bug del Signo**: El Excel humano mira el marcador numérico escrito a mano, pero la web usa estrictamente `pred.sign` aunque contradiga el marcador. (2) **Retraso IN_PLAY**: Un partido terminado puede seguir en `IN_PLAY` en la API, retrasando los puntos, mientras el Excel manual ya lo sumó. (3) **Cálculo de los 8 mejores terceros**: La web usa la rama en vivo (`api-data`) y calcula los 8 mejores terceros dinámicamente y al segundo basándose solo en grupos con partidos finalizados. El humano suele liarse con los empates a puntos y la diferencia de goles entre estos terceros, pero la web es infalible usando los criterios FIFA (Pts > DG > GF).
